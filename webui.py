...
class WebUI:
    def __init__(self, cfg) -> None:
        ...
        def _(_):
            if not self.sam_enabled.value:
                ...
                print(semantic_gaussian_mask)#이거 이미지를 저장해서 어떤 형식인지 파악한 후에 하드코딩으로 줄 수 있도록 해보기 # feat1
    ...
    def edit(self, edit_cameras, train_frames, train_frustums):
        ...
        elif self.guidance_type.value == "IDM-VTON":
            if not self.idmvton_ip2p:
                from threestudio.models.guidance.controlnet_guidance import (
                    ControlNetGuidance,
                )

                self.idmvton_ip2p = ControlNetGuidance(
                    OmegaConf.create({"min_step_percent": 0.05,
                                      "max_step_percent": 0.8,
                                        "control_type": "p2p"})
                )
            cur_2D_guidance = self.idmvton_ip2p
            print("using IDM-VTON")
"""
